...
def get_amr_ngrams(path, stat_save_path=None, N=3):
    data = []
    if stat_save_path:
    ...
    for line in read_amr(path):
        try:
            ...
            assert False
        amr.revert_of_edges()
        ngrams = amr.extract_ngrams(N, multi_roots=True) # dict(list(tuple))
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        sys.exit(0)
    #print('loading ...')
    hypothesis = get_amr_ngrams(sys.argv[1], N=int(sys.argv[3]))
    references = [[x] for x in get_amr_ngrams(sys.argv[2], N=int(sys.argv[3]))]
    ...
    for i, h in enumerate(hypothesis):
        print(corpus_bleu([references[i]], [h], weights=weights, smoothing_function=smoofunc, auto_reweigh=True))
