...
def build_arg_parser2():
    ...
    parser.add_option('--do_not_mark_quotes', action='store_true',
                      help=":op1 \"David\" will be treated same as :op1 David")
...
def main(arguments):
    ...
    while True:
        ...
        #print(cur_amr1)
        #print(cur_amr2)
        amr1 = amr.AMR.parse_AMR_line(cur_amr1, do_not_mark_quotes = arguments.do_not_mark_quotes)
        amr2 = amr.AMR.parse_AMR_line(cur_amr2, do_not_mark_quotes = arguments.do_not_mark_quotes)
        prefix1 = "a"
        prefix2 = "b"
        ...
        (best_mapping, best_match_num) = get_best_match(instance1, attributes1, relation1,
                                                        ...
                                                        prefix1, prefix2)
        #print(print_alignment(best_mapping, instance1, instance2))
        if verbose:
            log_helper.debug( "best match number", best_match_num)
...
#            print "Smatch score: %.3f" % best_f_score
    ...
    if verbose:
        ...
        log_helper.debug( total_match_num, total_test_num, total_gold_num)
        log_helper.debug( "---------------------------------------------------------------------------------")
...
if __name__ == "__main__":
    ...
    if args.v:
        ...
        log_helper = helpers.LogHelper(5)
    main(args)
