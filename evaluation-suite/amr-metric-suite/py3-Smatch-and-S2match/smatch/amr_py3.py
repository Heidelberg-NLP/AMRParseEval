...
try:
    ...
    import smatch.helpers


verbose = False

if verbose:
    log_helper = helpers.LogHelper(1)
"""
    ...
    def parse_AMR_line(line, do_not_mark_quotes = False):
            ...
            if c == "\"":
                ...
                if state == 2:
                    ...
                    if cur_relation_name != "":
                        log_helper.error( "Format error when processing ", line[0:i+1])
                        return None
                ...
                # or node value (variable) at this moment
                    ...
                    if len(parts) < 2:
                        log_helper.error( "Error in processing; part len < 2", line[0:i+1])
                        return None
                    ...
                    if len(stack) == 0:
                        log_helper.error( "Error in processing", line[:i], relation_name, relation_value)
                        return None
                ...
                if state == 1:
                    ...
                    if node_name in node_dict:
                        log_helper.error( "Duplicate node name ", node_name, " in parsing AMR")
                        return None
                    ...
                    log_helper.error( "Error in parsing AMR", line[0:i+1])
                    return None
                state = 3
            elif c == ")":
                if in_quote:
                ...
                if len(stack) == 0:
                    log_helper.error( "Unmatched parenthesis at position", i, "in processing", line[0:i+1])
                    return None
                # Last significant symbol is ":". Now we encounter ")"
                # Example:
                # :op2 "Brown") or :op2 w)
                ...
                if state == 2:
                    ...
                    parts = temp_attr_value.split()
                    if len(parts) < 2:
                        log_helper.error( "Error processing", line[:i+1], temp_attr_value)
                        return None
                    relation_name = parts[0].strip()
                    relation_value = parts[1].strip()
        ...
        for v in node_name_list:
            if v not in node_dict:
                print(v, node_name_list, node_dict)
                log_helper.error( "Error: Node name not found", v)
                return None
            else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        log_helper.error( "No file given")
        exit(1)
    amr_count = 1
    for line in open(sys.argv[1]):
        ...
        if cur_line == "" or cur_line.startswith("#"):
            continue
        log_helper.debug( "AMR", amr_count)
